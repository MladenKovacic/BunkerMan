...
public class Init
{
    static void Main()
    {
        ...
        var numberOfRations = int.Parse(Console.ReadLine());
        ...
        for (int i = 0; i < numberOfDays; i++)
        {
            if (i <= storage.FoodList.Count -1 )
            {
                ...
                if (kcal >= 2000)
                {
                    Console.WriteLine($"Day {i +1}: Ate | ({storage.FoodList[i].Name}) -> (base={storage.FoodList[i].BaseCalories}) -> (kcal={Math.Round(kcal)}) health change=+{Math.Round(kcal /200)} health={BunkerMan.Health}");
                }
                if (kcal < 2000)
                {
                    Console.WriteLine($"Day {i +1}: Ate | ({storage.FoodList[i].Name}) -> (base={storage.FoodList[i].BaseCalories}) -> (kcal={Math.Round(kcal)}) health change=-{Math.Round(kcal /100)} health={BunkerMan.Health}");
                }
            ...
            {
                BunkerMan.Health -= 20;
               Console.WriteLine($"Day {i +1}: No Food |                          -> kcal=0 health change = =-20  {BunkerMan.Health}  ");
            }
            if (i + 1 == numberOfDays)
            {
